...
项目内含 `obsidian/` 文件夹，可直接作为 Obsidian 仓库打开：
- `~/项目开发/上市公司数据/obsidian/` → Obsidian 打开此文件夹
- 需安装 Dataview 社区插件（已预配 .obsidian/ 配置）
- 使用 `python3 refresh.py` 刷新数据后重新生成 Obsidian 文件

## 改动记录

| 日期 | 版本 | 内容 |
|------|------|------|
| 2026-06-25 | v2.1 | Obsidian Vault：194家公司Markdown（YAML frontmatter）+ 3个Dataview看板；行业数据baostock补充5,014家覆盖率94%；全量检索看板（表格/卡片+排序+导出） |
| 2026-06-25 | v2.0 | P0急救 + 全量A股导入5,535家 + 精选看板V2（银行联动+CSV导出） |
| 2026-06-08 | v1.5 | 新增银行子模块（38家+12家外资银行） |
| 2026-06-07 | v1.0 | 初始版本 |
